...
{
    Transform transform;
    transform.position() = glm::vec3(m_positionX->value(), m_positionY->value(), m_positionZ->value());
    transform.scale() = glm::vec3(m_scaleX->value(), m_scaleY->value(), m_scaleZ->value());
    transform.setRotationEuler(
        glm::radians(m_rotationX->value()), glm::radians(m_rotationY->value()), glm::radians(m_rotationZ->value()));
    return transform;
}
