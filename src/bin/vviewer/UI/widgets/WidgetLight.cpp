...
WidgetLight::WidgetLight(QWidget * parent, vengine::ComponentLight& lightComponent) : QWidget(parent), m_lightComponent(lightComponent)
{
    ...
    if (lightComponent.light->type == LightType::POINT_LIGHT) m_lightTypeComboBox->setCurrentIndex(0);
    else if (lightComponent.light->type == LightType::DIRECTIONAL_LIGHT) m_lightTypeComboBox->setCurrentIndex(1);
...
{
    if (m_lightTypeComboBox->currentIndex() == 0) m_lightComponent.light->type = LightType::POINT_LIGHT;
    else if (m_lightTypeComboBox->currentIndex() == 1) m_lightComponent.light->type = LightType::DIRECTIONAL_LIGHT;
}
