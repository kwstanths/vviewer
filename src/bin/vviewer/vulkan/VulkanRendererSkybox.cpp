...
VulkanCubemap* VulkanRendererSkybox::createIrradianceMap(VulkanCubemap* inputMap, uint32_t resolution) const
{
    try {
        ...
        return new VulkanCubemap(inputMap->m_name + "irradiance", cubemapImage, cubemapMemory, cubemapImageView, cubemapSampler);
    }
    catch (std::runtime_error& e) {
        utils::ConsoleCritical("Failed to create a vulkan cubemap from an image: " + std::string(e.what()));
        return nullptr;
...
{
    {
        ...
        std::array<VkDescriptorSetLayoutBinding, 2> setBindings = { skyboxTextureLayoutBinding, irradianceTextureLayoutBinding };
